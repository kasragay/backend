...
type SwaggerUIConfig struct {
	FaviconURL32 string `json:"-"`
	FaviconURL16 string `json:"-"`
	...
	// default: false
	Filter SwaggerUIFilterConfig `json:"-"`
	...
	SyntaxHighlight *SwaggerUISyntaxHighlightConfig `json:"-"`
	...
	OAuth *SwaggerUIOAuthConfig `json:"-"`
...
type SwaggerUIFilterConfig struct {
	Enabled    bool
	Expression string
}

func (fc SwaggerUIFilterConfig) Value() interface{} {
	if fc.Expression != "" {
...
type SwaggerUISyntaxHighlightConfig struct {
...
func (shc SwaggerUISyntaxHighlightConfig) Value() interface{} {
	if shc.Activate {
		return shc
	}
	return false
...
var (
	ConfigDefault = SwaggerUIConfig{
		FaviconURL32: "https://api.ssenerg.com/v1/assets/favicon32x32.ico",
		FaviconURL16: "https://api.ssenerg.com/v1/assets/favicon16x16.ico",
		Title:        "Kasragay API Docs",
		...
		DocExpansion:             "list",
		SyntaxHighlight: &SwaggerUISyntaxHighlightConfig{
			Activate: true,
			Theme:    "agate",
		},
...
func swaggerConfigDefault(config ...SwaggerUIConfig) SwaggerUIConfig {
	...
	cfg := config[0]
	if cfg.FaviconURL32 == "" {
		cfg.FaviconURL32 = ConfigDefault.FaviconURL32
	}

	if cfg.FaviconURL16 == "" {
		cfg.FaviconURL16 = ConfigDefault.FaviconURL16
	}
...
func NewSwaggerUI(config ...SwaggerUIConfig) fiber.Handler {
	cfg := swaggerConfigDefault(config...)
